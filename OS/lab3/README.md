...
在高級部分，需要在EL1中使能中斷。 您只能禁用中斷來保護關鍵部分。 您可以使用以下代碼來啟用/禁用中斷。
...
### Advanced Exercises 1 - Timer Multiplexing

計時器可用於執行週期性作業（例如調度和日誌記錄）以及一次性執行（例如休眠和超時）。 然而，硬件定時器的數量是有限的。 因此，內核需要一種軟件機制來復用定時器。

一種簡單的方法是使用周期性計時器。 內核可以使用tick週期作為時間單位併計算相應的超時tick。 例如，假設週期性定時器的頻率為1000HZ，進程休眠1.5秒。 內核可以在當前tick之後1500tick的時刻添加喚醒事件。

然而，當節拍頻率太低時，定時器的分辨率就會變差。 那麼，它就不能用於時間敏感的工作。 當節拍頻率太高時，會為冗餘定時器中斷處理引入大量開銷。

另一種方法是使用一次性計時器。 當有人需要超時事件時，就會將計時器插入到計時器隊列中。 如果超時時間早於前一個編程的到期時間，則內核會將硬件計時器重新編程為較早的時間。 在定時器中斷處理程序中，它執行過期定時器的回調函數。

在這一高級部分中，您需要實現定時器 API，用戶可以使用一次性定時器（核心定時器是一次性定時器）在超時時註冊回調函數。 API 及其用例應類似於以下偽代碼。
...
要測試 API，您需要實現 shell 命令 setTimeout MESSAGE SECONDS。 它會在 SECONDS 後打印 MESSAGE，其中包含當前時間和命令執行時間。

> Todo
> 
> 使用計時器 API 實現 setTimeout 命令。

> Important
> 
> setTimeout 是非阻塞的。 用戶可以設置多個超時。 打印順序由命令執行時間和用戶指定的 SECONDS 決定。

> Hint
> 
> 您可以通過修改特權寄存器或僅在 EL1 中運行程序來實現此練習。

### Advanced Exercise 2 - Concurrent I/O Devices Handling

內核需要同時處理大量的I/O設備。 對於處理時間較短的設備（例如 UART），內核可以在準備就緒後立即完成其處理程序。 然而，對於那些需要較長時間進行後續處理的設備（例如網絡接口控制器），內核需要調度執行順序。

通常，我們希望使用先到先得的原則來防止飢餓。 然而，我們可能還希望優先執行一些關鍵處理程序。 在這一部分中，您需要知道如何使用單個線程（即單個堆棧）來實現它。

#### Decouple the Interrupt Handlers

實現中斷處理程序的一種更簡單的方法是在禁用中斷的情況下一次處理一個設備的所有數據。 然而，不太關鍵的中斷處理程序可能會長時間阻塞更關鍵的中斷處理程序。 因此，我們希望將中斷處理程序和實際處理解耦。

這可以通過任務隊列來實現。 在中斷處理程序中，內核
1. 屏蔽設備的中斷線，
2. 通過 DMA 從設備緩衝區移動數據，或手動複製，
3. 將處理任務放入事件隊列中，
4. 執行啟用中斷的任務，
5. 取消屏蔽中斷線以在任務結束時獲取下一個中斷。

隊列中的任務可以在系統空閒時處理。 此外，內核可以按任何順序（例如 FIFO 或 LIFO）執行任務。

> Todo
> 
> 實現任務隊列機制，以便中斷處理程序可以將其處理任務添加到其中。

#### Nested Interrupt

隊列中的任務可以隨時執行，但我們希望它們盡快執行。 這是因為高優先級的進程可能正在等待數據。

因此，在中斷處理程序返回到用戶程序之前，應該在中斷允許的情況下執行中斷上下文中的任務（否則，關鍵中斷將被阻塞）。 然後，可以嵌套中斷處理程序。 因此，除了通用寄存器外，還應該保存 spsr_el1 和 elr_el1，以便保留之前保存的數據。

> Todo
> 
> 在啟用中斷的情況下返回到用戶程序之前執行隊列中的任務。

#### Preemption

現在，任何中斷處理程序都可以搶占任務的執行，但新入隊的任務仍然需要等待當前正在運行的任務完成。 如果新入隊的優先級較高的任務能夠搶占當前正在運行的任務，那就更好了。

為了實現搶占，內核可以在返回上一個中斷處理程序之前檢查最後執行的任務的優先級。 如果有更高優先級的任務，則執行最高優先級的任務。
