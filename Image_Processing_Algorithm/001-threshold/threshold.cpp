...
int main() {
	...
	Mat original = imread(imgname, IMREAD_GRAYSCALE);
	Mat image = imread(imgname, IMREAD_GRAYSCALE);

	//binary
	Mat image2 = imread(imgname, IMREAD_GRAYSCALE);
	...
	//binary inverse
	Mat image3 = imread(imgname, IMREAD_GRAYSCALE);
	for (int i = 0; i < image2.rows; i++) {
		for (int j = 0; j < image2.cols; j++) {
	...
	//trunc
	Mat image4 = imread(imgname, IMREAD_GRAYSCALE);
	...
	//tozero
	Mat image5 = imread(imgname, IMREAD_GRAYSCALE);
	...
	//tozero inv
	Mat image6 = imread(imgname, IMREAD_GRAYSCALE);
	...
	int inc = ancho / 256;
	for (int i = 0; i < 255; i++) {
		rectangle(myMat1, Point(inc * i, myMat1.rows), Point((inc*(i + 1) - 1), myMat1.rows - ((arr[i] * myMat1.rows) / (max))), Scalar(255, 255, 255, 0), cv::FILLED);
	}
	...
	for (int i = 0; i < 255; i += 2) {
		rectangle(myMat2, Point(inc * i, myMat2.rows), Point((inc*(i + 2) - 1), myMat2.rows - (((arr[i] + arr[i + 1]) * myMat2.rows) / (2 * max))), Scalar(255, 255, 255, 0), cv::FILLED);
	}
	...
	for (int i = 0; i < 255; i += 4) {
		rectangle(myMat4, Point(inc * i, myMat4.rows), Point((inc*(i + 4) - 1), myMat4.rows - (((arr[i] + arr[i + 1] + arr[i + 2] + arr[i + 3]) * myMat4.rows) / (4 * max))), Scalar(255, 255, 255, 0), cv::FILLED);
	}
